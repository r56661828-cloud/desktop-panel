...
### 3.3 自动更新（NSIS 安装版，docs/TECH-DESIGN-UPDATE.md）

安装版内置自动更新（electron-updater + GitHub Releases feed）：

- 启动 30s 后与每 4 小时自动检查；发现新版本后台静默下载，标题栏出现「更新 v*」；
- 点击「更新」→ 弹框展示更新内容与后果（重启安装、草稿保护）→「立即升级」静默安装并自动重启；
- 点「暂不」= 跳过该版本（不再自动下载/弹框，托盘「检查更新」可手动触发）；
- 已下载未安装时强杀进程 → 下次启动直接静默升级进新版，并自动打开「更新内容」标签；
- **绿色版（zip）无自动更新**，请到 Releases 页手动下载替换。

### 3.4 安装后的功能自检（对齐 TEST-CASES P0，约 10 分钟）
...
1. **未签名 → Windows 11「智能应用控制」直接拦截（实测 2026-09-12，v0.1.2 真机复现 2026-09-13）**：双击 exe 弹「智能应用控制已阻止可能不安全的应用」，**没有「仍要运行」入口**（普通 SmartScreen 的「更多信息→仍要运行」不适用），且与重装次数无关（SAC 按文件签名+云信誉评估，不认安装状态）。处理：
   ...
   - ⚠️ CI 日志排查提示：electron-builder 打包时打印的 `signing with signtool.exe` 是**无条件入口日志**，未配置证书时实际静默跳过签名——不要据此误判产物已签名；
